...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const newMailExists = await this.usersRepository.findByEmail(email);

    if (!user) {
      throw new _AppError.default('User not found');
    }

    if (newMailExists && newMailExists.id !== user.id) {
      throw new _AppError.default('Email already in use, sorry! try another.');
    }
    ...
    if (password && !old_password) {
      throw new _AppError.default('You must type your old passaword');
    }
    ...
    if (password && old_password) {
      ...
      if (!checkOldPassword) {
        throw new _AppError.default('You typed the wrong password');
      }
