...
let ListProviderAppointmentsService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  constructor(appointmentsReposiotry, cacheProvider) {
    this.appointmentsReposiotry = appointmentsReposiotry;
  ...
  async execute({
    ...
    if (!appointments) {
      appointments = await this.appointmentsReposiotry.findAllInDayFromProvider({
        provider_id,
        day,
        month,
        year
